...
const INITIAL_WALL_WAIT_MS = 250;
...
const defaultTargets: WallHostTarget[] = [
	{
		id: "infra",
		name: "infra",
		baseUrl: "http://127.0.0.1:9580",
		roleFallback: "wall-host",
	},
	{
		id: "proto",
		name: "proto",
		baseUrl: "http://192.168.178.216:9580",
		roleFallback: "wall-host",
	},
	{
		id: "blep",
		name: "blep",
		baseUrl: "http://192.168.178.164:9580",
		roleFallback: "wall-host",
	},
	{
		id: "mac",
		name: "mac",
		baseUrl: "http://192.168.178.107:9580",
		roleFallback: "wall-host",
	},
];
...
export async function getWallCollectorData(): Promise<WallCollectorResult> {
	...
	return buildLoadingCollector();
}

function buildLoadingCollector(): WallCollectorResult {
	return {
		status: "loading",
		error: null,
		stale: false,
		generatedAt: "Systeemstatus wordt geladen",
		statusLabel: "Wall-feed laden",
		connectionLabel: "Hosts controleren",
		cpuAggregatePercent: 0,
		cpuAggregateSeries: [],
		hosts: [],
	};
}

function buildErrorCollector(
	error: string,
	previous?: WallCollectorResult | null,
): WallCollectorResult {
	if (previous?.hosts.length) {
		return {
			...previous,
			status: "live",
			error,
	...
	return {
		...
		cpuAggregatePercent: 0,
		cpuAggregateSeries: previous?.cpuAggregateSeries ?? [],
		hosts: previous?.hosts ?? [],
	};
}

async function waitForInitialRefresh(
	refreshPromise: Promise<WallCollectorResult>,
) {
	return new Promise<WallCollectorResult>((resolve) => {
		const timer = setTimeout(() => {
			resolve(buildLoadingCollector());
		}, INITIAL_WALL_WAIT_MS);
		timer.unref?.();
...
async function refreshCollector(
	...
	state.refreshPromise = collectWallData(state)
		...
		.catch((error) => {
			const fallback = buildErrorCollector(
				error instanceof Error
					? error.message
					: "Systeemstatus ophalen mislukt",
				state.cache,
			);
...
async function collectWallData(
	state: CollectorState,
): Promise<WallCollectorResult> {
	...
	const cpuAggregatePercent = results.reduce(
		(total, result) => total + result.cpuPercent,
		0,
	);
	const overallState = hosts.reduce<HealthState>((current, host) => {
		return severityRank(host.state) > severityRank(current)
			? host.state
			: current;
	}, "healthy");
	const cpuAggregateSeries = pushHistoryPoint(
		state.cpuAggregateSeries,
		cpuAggregatePercent,
	);
...
function getWallTargets(): WallHostTarget[] {
	...
	const targets = raw
		...
		.filter(Boolean)
		.map((item) => {
			const [id, baseUrl, roleFallback] = item
				.split("|")
	...
	try {
		const snapshot = await withTimeout(
			(async () => {
				...
				return (await response.json()) as WallnodeSnapshot;
			})(),
			REQUEST_TIMEOUT_MS,
			() => controller.abort(),
		);
	...
	return new Promise<T>((resolve, reject) => {
		const timer = setTimeout(() => {
			try {
				...
				// Best effort only. The timeout should still reject the request.
			}
...
function mapSnapshotToHost(
	target: WallHostTarget,
	snapshot: WallnodeSnapshot,
): HostStatus {
	...
	const cpuTempC =
		snapshot.cpuTempC === null ? null : roundTemperature(snapshot.cpuTempC);
...
function formatLoad(load: WallnodeSnapshot["load"]) {
	return [load["1m"], load["5m"], load["15m"]]
		.map((value) =>
			value === null ? "n/a" : value.toFixed(2).replace(/\.00$/, ""),
		)
		.join(" / ");
}
