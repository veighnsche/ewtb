...
describe("getWallCollectorData", () => {
	...
	it("returns a loading snapshot while the first refresh runs, then serves the cached wall state", async () => {
		process.env.WALLNODE_TARGETS = "alpha|http://alpha.test:9580|wall-host";
		...
		const first = await getWallCollectorData();
		...
		const second = await getWallCollectorData();
	...
	it("times out a hanging host request instead of leaving the wall collector pending forever", async () => {
		process.env.WALLNODE_TARGETS = "alpha|http://alpha.test:9580|wall-host";
		vi.useFakeTimers();
		...
		const firstPromise = getWallCollectorData();
		...
		expect(second.error).toBeNull();
		expect(second.stale).toBe(false);
		expect(second.connectionLabel).toBe("0/1 hosts online");
		expect(second.statusLabel).toBe("Wall-feed vraagt aandacht");
		expect(second.hosts[0]).toMatchObject({
			id: "alpha",
			state: "critical",
			error: "Host reageert niet op tijd",
			uptime: "offline",
		});
		expect(fetchMock).toHaveBeenCalledTimes(1);
	});
